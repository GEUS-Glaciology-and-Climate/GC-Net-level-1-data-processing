...
obs_df = pd.read_excel('./metadata/GCNet_maintenance.xlsx', 
                       sheet_name=sitename, engine='openpyxl')
print(obs_df)
...
df_ks = pd.read_csv("C://Users/bav/OneDrive - Geological survey of Denmark and Greenland//Data/AWS/GC-Net//20190501/"+ID+"c.dat_Req1957",skiprows = 54, sep = ' ')

header = pd.read_csv("C://Users/bav/OneDrive - Geological survey of Denmark and Greenland//Data/AWS/GC-Net//20190501/"+ID+"c.dat_Req1957",nrows = 53, header = None)
...
df_ks['date'] =  pd.to_datetime(  year +'-' + juliandate, format='%Y-%j') + np.array([pd.Timedelta(days = x) for x in day_frac.astype(float).values])
...
df_org = pd.read_csv("C://Users/bav/GitHub/JEB_GC-Net-master/output/"+ID+"c.dat",header=None, delim_whitespace=True)
df_org[df_org==999] = np.nan
df_org.columns = ['ID', 'year', 'DoY', 'ISWR', 'OSWR', 'NSWR', 'TA1', 'TA2', 'TA3', 'TA4', 'RH1', 'RH2', 'VW1', 'VW2', 'DW1', 'DW2', 'P', 'HS1', 'HS2', 'TS1', 'TS2', 'TS3', 'TS4', 'TS5', 'TS6', 'TS7', 'TS8', 'TS9', 'TS10', 'V','VW1_std', 'VW2_std', 'HW1', 'HW2', 'Alb','unkown', 'QC1', 'QC2', 'QC3', 'QC4']
...
day_frac = (df_org.iloc[:,2] - np.floor(df_org.iloc[:,2]))
df_org['date'] =  pd.to_datetime(  year +'-' + juliandate, format='%Y-%j') + np.array([pd.Timedelta(days = x) for x in day_frac.astype(float).values])
...
plt.plot(obs_df["Date (dd-mm-yyyy HH:MM)"],obs_df['W1 before (cm)']/100,'*',
         markerfacecolor='none', markersize=26,c='C0',label='HW1 obs')
plt.plot(obs_df["Date (dd-mm-yyyy HH:MM)"],obs_df['W2 before (cm)']/100,'*',
         markerfacecolor='none', markersize=26,c='C1',label='HW2 obs')

plt.plot(obs_df["Date (dd-mm-yyyy HH:MM)"],obs_df['T1 before (cm)']/100,'o',
         markerfacecolor='none', markersize=26,c='C0',label='HT1 obs')
plt.plot(obs_df["Date (dd-mm-yyyy HH:MM)"],obs_df['T2 before (cm)']/100,'o',
         markerfacecolor='none', markersize=26,c='C1',label='HT2 obs')
