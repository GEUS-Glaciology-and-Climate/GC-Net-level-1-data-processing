...
for site, ID in zip(site_list.Name, site_list.ID):
    ...
    for year in range(1998, 2005):
        print(year)
        dfc_raw = dfc_raw.append(pd.read_csv('C:/Users/bav/GitHub/JEB_GC-Net-master/output/' + str(ID).zfill(2)+'_'+str(year)+'c.dat', header = None, delim_whitespace=True))
        
    dfc_raw.columns = c_file_header_str[:len( dfc_raw.columns)]
    dfc_raw['date'] = pd.to_datetime(dfc_raw['year'], format='%Y') + pd.to_timedelta(dfc_raw['DoY'] - 1, unit='d')
    ...
    l0inipath = "./L0N_ini/"
    cfiledir = 'L0/'+str(ID).zfill(2)+'-'+site+'/C file/'+str(ID).zfill(2)+'c.dat'
